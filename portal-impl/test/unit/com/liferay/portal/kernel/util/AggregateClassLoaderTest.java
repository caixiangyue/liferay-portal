...
public class AggregateClassLoaderTest {
	...
	public void testAddClassLoader() {
		_testAddClassLoader(
			Arrays.asList(_testClassLoader2), _testClassLoader1,
			new ClassLoader[] {_testClassLoader1, _testClassLoader2});

		_testAddClassLoader(
			Arrays.asList(_testClassLoader1, _testClassLoader2), null,
			new ClassLoader[] {_testClassLoader1, _testClassLoader2});

		_testAddClassLoader(
			Arrays.asList(_testClassLoader2), null,
			new ClassLoader[] {_testClassLoader2, _testClassLoader2});

		_testAddClassLoader(
			Arrays.asList(_testClassLoader1), null,
	...
	public void testGetAggregateClassLoader() {
		...
		_assertAggregateClassLoader(
			_testClassLoader1, Arrays.asList(_testClassLoader2),
			(AggregateClassLoader)AggregateClassLoader.getAggregateClassLoader(
				new ClassLoader[] {_testClassLoader1, _testClassLoader2}));
		...
		_assertAggregateClassLoader(
			_testClassLoader1, Collections.emptyList(),
			(AggregateClassLoader)AggregateClassLoader.getAggregateClassLoader(
	...
	public void testGetResources() throws Exception {
		...
		try {
			...
			Assert.assertTrue(
				"getResources() throws NullPointerException if parent class " +
					"loader was null",
				ioe.getCause() instanceof NullPointerException);
		}
		...
		try {
			...
			Assert.assertTrue(
				...
				ioe.getCause() instanceof IOException);
		}
	}
	...
	private void _testAddClassLoader(
		List<ClassLoader> expectedClassLoaders, ClassLoader parentClassLoader,
